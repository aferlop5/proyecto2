...
from modelsFlaskAlchemy import db, Robots, EstadoRobot
...
# Sustituye la función set_modo_auto por esta versión que lanza un hilo
@robots.route("/robots/<int:id>/modo/auto", methods=["POST"])
def set_modo_auto(id):
    global auto_thread
    if auto_thread and auto_thread.is_alive():
        return jsonify({"error": "Automático ya en marcha"}), 409

    pause_event.clear()
    stop_event.clear()
    auto_thread = threading.Thread(target=routine_auto, args=(id,), daemon=True)
    auto_thread.start()
...
def publicar_estado(state_dict):
    ...
    belt_speed = state_dict.get("belt_speed", 0)  # Obtener el valor de belt_speed
    estado = EstadoRobot(
        ...
        belt_speed=belt_speed,  # Añadir belt_speed al constructor
    ...
    db.session.commit()
    print(f"[INFO] Estado publicado: {estado}")
...
progress_data = {"small": 0, "large": 0, "offset_idx": 0}

# Añade estos endpoints para controlar el hilo automático

@robots.route("/robots/<int:id>/auto/pause", methods=["POST"])
def pause_auto(id):
    pause_event.set()
    return jsonify({"message": "Pausa solicitada"})

@robots.route("/robots/<int:id>/auto/resume", methods=["POST"])
def resume_auto(id):
    if not auto_thread or not auto_thread.is_alive():
        return jsonify({"error": "Automático no está en marcha"}), 409
    pause_event.clear()
    return jsonify({"message": "Reanudado"})

@robots.route("/robots/<int:id>/auto/stop", methods=["POST"])
def stop_auto(id):
    stop_event.set()
    return jsonify({"message": "Paro solicitado"})

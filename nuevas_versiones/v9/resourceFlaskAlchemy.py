...
socketio = SocketIO()  # Ajusta la inicialización según tu configuración
...
def set_modo_auto(id):
    ...
    try:
        ...
        # Arranca la tarea de actualización de sensores cada 0.5 s
        auto_flag = [True]
        socketio.start_background_task(sensor_updater, robot, DI5, DI1, auto_flag)
        ...
        while small_pieces < 3 or large_pieces < 3:
            ...
            if robot.digital_read(DI1) == PinState.LOW:
                ...
                robot.stop_conveyor(conveyor_id)
                large_pieces += 1
                emit_counters_update(small_pieces, large_pieces)
            else:
        ...
        # Detener la tarea de sensores al finalizar el modo automático
        auto_flag[0] = False

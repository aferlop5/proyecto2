from control import get_sensor_states, get_elapsed_time
...
def auto():
    ...
    try:
        ...
        if result is None:
            return jsonify({'error': 'No se pudo iniciar el modo automático por falta de conexión con el robot.'}), 500

        ventosa, sensordi1, sensordi5, robot_data = result
...
def get_timer():
    try:
        ...
        return jsonify({'error': 'Could not retrieve timer'}), 500
    ...
    while True:
        try:
            if robot and robot.is_connected():  # Check connection
                ...
                cursor = connection.cursor()
                cursor.execute("""
                    INSERT INTO posiciones_robot (x, y, z, roll, pitch, yaw, connected, timestamp)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                """, (*position, connected, timestamp))
                connection.commit()
                cursor.close()
                connection.close()
...
position_thread = threading.Thread(target=read_and_save_robot_position)

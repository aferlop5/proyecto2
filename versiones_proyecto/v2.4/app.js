...
function updateRobotPositionDisplay(data) {
    ...
    if (positionDisplay) {
        if (data.error) {
            positionDisplay.textContent = data.error;
            positionDisplay.style.color = "red";
        } else {
            positionDisplay.textContent = `Posición: X=${data.x}, Y=${data.y}, Z=${data.z}, Roll=${data.roll}, Pitch=${data.pitch}, Yaw=${data.yaw}`;
            positionDisplay.style.color = "";
        }
    }
}
        ...
        .then(data => {
            if (data.error) {
                ...
                if (elapsedTimeElement) {
                    elapsedTimeElement.textContent = "0 segundos";
                    elapsedTimeElement.style.color = "red";
                    elapsedTimeElement.style.animation = "blink 1s step-start infinite";
                }
                throw new Error(data.error);
            }
...
// Function to fetch and display the timer data
async function updateTimerDisplay() {
    try {
        ...
        const elapsedTimeElement = document.getElementById('elapsedTime');
        if (elapsedTimeElement) {
            elapsedTimeElement.textContent = `${data.elapsed_time.toFixed(2)} segundos`;
        }
    } catch (error) {
